...
spark-submit --class com.hadooparchitecturebook.spark.dedup.GenDedupInput $JARFILE \
  ded_input/ded_100000.txt 100000 100 2>&1 | tee $OUTPUTDIR/GenDedupInput.out

echo "Deduping - Generating data"
spark-submit --class com.hadooparchitecturebook.spark.dedup.GenDedupInput $JARFILE \
  ded_input/ded_10.txt 10 10 2>&1 | tee $OUTPUTDIR/GenDedupInput.out

echo "Deduping - Executing example"
spark-submit --class com.hadooparchitecturebook.spark.dedup.SparkDedupExecution $JARFILE \
